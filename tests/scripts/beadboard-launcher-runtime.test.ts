...
test('start falls back to repoRoot when runtimeRoot exists but has no dev script', async () => {
  ...
  try {
    ...
    // Must fall back to repoRoot since the fake runtimeRoot has no package.json with a dev script
    assert.notEqual(payload.startRoot, fakeRuntimeRoot);
  } finally {
...
test('start uses runtimeRoot when it contains a package.json with a dev script', async () => {
  ...
  try {
    ...
    assert.equal(payload.ok, true);
    assert.equal(payload.command, 'start');
    // Must prefer the fakeRuntimeRoot since it has a valid package.json with a dev script
    assert.equal(payload.startRoot, fakeRuntimeRoot);
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
});
